"""Tests for the W4 ToolProfile dispatch in penpot-api-mcp.
...
def _test_settings(**overrides: Any) -> PenpotSettings:
...
def test_profiles_py_exists() -> None:
    """``penpot_api_mcp/tools/profiles.py`` must exist."""
    import penpot_api_mcp.tools.profiles  # noqa: F401
...
def test_profiles_py_defines_group_registry() -> None:
    ...
    expected = {
        ...
        ("project_tools", "register_project_tools_for_profile"),
        ("file_tools", "register_file_tools_for_profile"),
        ("object_tools", "register_object_tools_for_profile"),
    }
...
def test_profiles_py_defines_apply_penpot_api_tool_profile() -> None:
    """``apply_penpot_api_tool_profile`` is async (returns a coroutine fn)."""
    assert callable(apply_penpot_api_tool_profile)
    assert asyncio_iscoroutinefunction(apply_penpot_api_tool_profile)


def test_profiles_py_references_correct_env_var() -> None:
    """The dispatch helper is called with ``PENPOT_API_TOOL_PROFILE``."""
    import inspect

    source = inspect.getsource(apply_penpot_api_tool_profile)
    assert "PENPOT_API_TOOL_PROFILE" in source
...
def test_register_xxx_tools_for_profile_exist() -> None:
    """All three ``_for_profile`` wrappers are exposed."""
    for attr in (
        "register_project_tools_for_profile",
        "register_file_tools_for_profile",
        "register_object_tools_for_profile",
    ):
        assert hasattr(tools_pkg, attr), f"missing {attr}"
        assert callable(getattr(tools_pkg, attr))
...
def test_profiles_uses_async_helper_not_sync_wrapper() -> None:
    """``apply_penpot_api_tool_profile`` calls ``_apply_tool_profile``,
...
def test_server_awaits_apply_penpot_api_tool_profile() -> None:
    """AST guard: ``server.py`` MUST ``await apply_penpot_api_tool_profile``.

    Structural check for ``ast.Await(value=ast.Call(func=ast.Name(
    id='apply_penpot_api_tool_profile')))``. NOT just a count of calls
    (the W3.2 round-1 lesson — count-only guards produce false
    positives that silently break in event loops).
    """
    from pathlib import Path

    server_path = Path(__file__).resolve().parents[2] / "penpot_api_mcp" / "server.py"
    ...
    def _walk(node: ast.AST) -> bool:
        if isinstance(node, ast.Await) and isinstance(node.value, ast.Call):
            call = node.value
            func = call.func
            if isinstance(func, ast.Name) and func.id == "apply_penpot_api_tool_profile":
                return True
    ...
    assert _walk(tree), (
        "server.py must await apply_penpot_api_tool_profile (the W2b.3 "
...
def test_server_does_not_call_sync_apply_tool_profile() -> None:
    ...
    assert "apply_tool_profile(" not in stripped, (
        ...
        "_apply_tool_profile via apply_penpot_api_tool_profile)."
    )
...
def test_guard_fails_when_await_is_removed() -> None:
    ...
    Build a synthetic module that calls ``apply_penpot_api_tool_profile``
    ...
    source = """
async def bad():
    apply_penpot_api_tool_profile(server, settings, client)
"""
...
def test_caller_supplied_settings_are_preserved(
    ...
    """Monkey-patch ``PenpotSettings.__init__`` so any env reload raises.
    ...
    original_init = PenpotSettings.__init__

    def tracking_init(self: PenpotSettings, *args: Any, **kwargs: Any) -> None:
        init_call_count["n"] += 1
        original_init(self, *args, **kwargs)

    monkeypatch.setattr(PenpotSettings, "__init__", tracking_init)
    settings = _test_settings()
...
async def test_lifespan_actually_calls_client_close_on_shutdown() -> None:
    ...
    server, monkey-patch ``PenpotClient.close``, and verify the close
    ...
    original_close = PenpotClient.close

    async def tracking_close(self: PenpotClient) -> None:
        close_called["n"] += 1
        await original_close(self)

    with patch.object(PenpotClient, "close", tracking_close):
        ...
        await result._penpot_client.close()
...
def test_essential_tool_names_subset_check_enforced() -> None:
    """Source-level: ``apply_penpot_api_tool_profile`` passes
...
async def test_minimal_profile_registers_only_health_check() -> None:
    ...
    The W4 spec: no Penpot tools at MINIMAL (they would fail without
    credentials / a reachable Penpot instance). Strict equality on
    tool names — unreported extras fail loud.
    """
    settings = _test_settings()
    server = _fresh_server()
    with patch.dict("os.environ", {"PENPOT_API_TOOL_PROFILE": "minimal"}):
        await apply_penpot_api_tool_profile(server, settings, client=object())  # type: ignore[arg-type]
...
async def test_standard_profile_registers_all_groups() -> None:
    ...
    settings = _test_settings()
    server = _fresh_server()
    with patch.dict("os.environ", {"PENPOT_API_TOOL_PROFILE": "standard"}):
        await apply_penpot_api_tool_profile(server, settings, client=object())  # type: ignore[arg-type]
    names = {tool.name for tool in await server.list_tools()}
    expected = {
        # health
...
async def test_full_profile_registers_all_groups() -> None:
    ...
    settings = _test_settings()
    server = _fresh_server()
    with patch.dict("os.environ", {"PENPOT_API_TOOL_PROFILE": "full"}):
        await apply_penpot_api_tool_profile(server, settings, client=object())  # type: ignore[arg-type]
    names = {tool.name for tool in await server.list_tools()}
    expected = {
        "health_check",
...
async def test_create_app_full_profile_real_path() -> None:
    ...
    equality on tool names — every Penpot tool + ``health_check`` +
...
async def test_create_app_invalid_profile_raises() -> None:
    """A SET-BUT-INVALID ``PENPOT_API_TOOL_PROFILE`` raises
    ``InvalidProfileError`` (per the W0 spec — UNSET falls through to
    FULL, but a bogus value is loud)."""
    settings = _test_settings()
    server = _fresh_server()
    with patch.dict("os.environ", {"PENPOT_API_TOOL_PROFILE": "bogus-value"}):
        with pytest.raises(InvalidProfileError):
            await apply_penpot_api_tool_profile(
...
def asyncio_iscoroutinefunction(fn: Any) -> bool:
    """Cheap stand-in for ``asyncio.iscoroutinefunction`` so the import
    is not required at module load time."""
    import asyncio
