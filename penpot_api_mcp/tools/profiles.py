"""Tool profile registration groups for penpot-api-mcp MCP server.
...
``PENPOT_API_TOOL_PROFILE`` environment variable.
...
Profile tiers (Tier-A trivial — 3-tier mapping per the W4 plan):
    ...
    MINIMAL:  Only the MCP ``health_check`` tool + the ``discover_tools``
              ...
              deployments where the 6 PenpotClient-bound tools would
              fail (no credentials, no Penpot instance reachable).
    STANDARD: All 6 penpot-api-mcp tools + ``discover_tools`` (Tier-A
              trivial: same set as FULL — there's no "core subset" to
              drop at STANDARD).
    FULL:     All 6 penpot-api-mcp tools + ``discover_tools`` (registered
              via the ``register_all_fn`` bulk registration path).

The dispatch surface (``PROFILE_REGISTRATIONS`` + ``REGISTRATION_MAP`` +
``register_all_tool_groups`` + ``apply_penpot_api_tool_profile``) is
consumed by ``penpot_api_mcp.server.create_app`` which delegates to
...
# attribute name on the ``penpot_api_mcp.tools`` package. THIS is the
...
# ``getattr(penpot_api_mcp.tools, attr_name)`` — no name-specific
...
def _build_registration_map(
    settings: PenpotSettings, client: PenpotClient
    ...
    ``penpot_api_mcp.tools`` package (no hard-coded name-specific
    ...
    Args:
        settings: The caller-supplied ``PenpotSettings`` instance to
        ...
        client: The pre-constructed ``PenpotClient`` instance. Same
    ...
    Returns:
        Mapping from group key (e.g. ``"project_tools"``) to a single-arg
        async-or-sync callable that takes the ``FastMCP`` server.
    """
    from penpot_api_mcp import tools as _tools_module
...
def register_all_tool_groups(
    server: FastMCP, settings: PenpotSettings, client: PenpotClient
) -> None:
    """Bulk register every penpot-api-mcp tool group (called at FULL profile).

    Iterates ``_GROUP_REGISTRY`` directly — no name-specific
    conditionals. Each entry's ``attr_name`` is looked up dynamically on
    the ``penpot_api_mcp.tools`` package. Used as ``register_all_fn``
    for the W0 helper.

    Args:
        server: FastMCP server instance.
        settings: The caller-supplied ``PenpotSettings`` to pass through
            to every group registration (NOT re-loaded from env).
        client: The pre-constructed ``PenpotClient`` to bind into every
            group registration.
    """
    from penpot_api_mcp import tools as _tools_module
...
async def apply_penpot_api_tool_profile(
    server: FastMCP, settings: PenpotSettings, client: PenpotClient
) -> None:
    """Apply the ``PENPOT_API_TOOL_PROFILE`` dispatch to ``server`` at startup.

    Async because the W0 helper is async; called from
    ``penpot_api_mcp.server.create_app`` via
    ...
    await _apply_tool_profile(
        server,
        profile_env_var="PENPOT_API_TOOL_PROFILE",
